...
public class BrewingStandHandler extends BlockHandler<BrewingStandBlockEntity, BrewingStandScreenHandler> {
	public BrewingStandHandler() {
		super(BrewingStandBlockEntity.class, BrewingStandScreenHandler.class, "container.brewing");
	}

	@Override
	public Text handle(BlockPos clickedPos, BrewingStandScreenHandler container,
			BrewingStandBlockEntity blockEntity, BlockView world,
			BiConsumer<BlockPos, BrewingStandBlockEntity> saveMethod) throws HandlerException {
		Inventory brewingInventory = ReflectionUtils.findAndGetPrivateField(
		...
		return new TranslatableText("wdl.messages.onGuiClosedInfo.savedTileEntity.brewingStand");
	}
}
