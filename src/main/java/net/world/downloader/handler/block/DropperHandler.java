...
public class DropperHandler extends BlockHandler<DropperBlockEntity, Generic3x3ContainerScreenHandler> {
	public DropperHandler() {
		super(DropperBlockEntity.class, Generic3x3ContainerScreenHandler.class, "container.dropper");
	}

	@Override
	public Text handle(BlockPos clickedPos, Generic3x3ContainerScreenHandler container,
			DropperBlockEntity blockEntity, BlockView world,
			BiConsumer<BlockPos, DropperBlockEntity> saveMethod) throws HandlerException {
		Inventory dropperInventory = ReflectionUtils.findAndGetPrivateField(
		...
		return new TranslatableText("wdl.messages.onGuiClosedInfo.savedTileEntity.dropper");
	}
}
