...
/**
 * Attempts to reconstruct information about a MapState that isn't present.
 *
...
public final class MapDataHandler {
	 ...
	 *
	 * Note that we can't use the enum in MapIcon because it didn't exist
	 * in 1.9.
	 */
	@VisibleForTesting
	public static final byte DECORATION_PLAYER = 0, DECORATION_ITEM_FRAME = 1, DECORATION_OFF_PLAYER = 6;
	/**
	 * Additionally, note that this decoration was added in 1.11.
	 */
	@VisibleForTesting
	public static final byte DECORATION_FAR_OFF_PLAYER = 7;
	/**
	 ...
	 * @param mapData The MapState to fix.
	 * @param player {@link WDL#player}.
	 * @return The MapState to save, though currently it is the same reference as the parameter.
	 */
	public static MapDataResult repairMapData(int mapID, @Nonnull MapState mapData,
	...
	@Nullable
	private static MapDataResult checkPlayerHasMap(int mapID, MapState mapData, PlayerEntity player) {
		...
		List<MapIcon> playerDecorations = mapData.icons.values().stream()
				.filter(dec -> dec.getTypeId() == DECORATION_PLAYER)
		...
		MapIcon playerDecoration = playerDecorations.get(0);
		...
		if(!mapInInventory)
			mapInInventory = isMapWithID(player.getOffHandStack(), mapID);
	...
	@Nullable
	private static MapDataResult checkFrameHasMap(int mapID, MapState mapData, World world) {
		List<MapIcon> frameDecorations = mapData.icons.values().stream()
				.filter(dec -> dec.getTypeId() == DECORATION_ITEM_FRAME)
				.collect(Collectors.toList());
		...
		for (Entity e : entities) {
			...
			BlockPos framePos = frame.getDecorationBlockPos();
			...
			for (MapIcon decoration : frameDecorations) {
				if (decoration.getX() == iconX && decoration.getZ() == iconZ) {
	...
	public static class MapDataResult {
		MapDataResult(MapState map, @Nullable Entity confirmedOwner, @Nullable MapIcon decoration) {
			this.map = map;
		 ...
		 */
		@Nullable
		public final MapIcon decoration;
		/**
		...
		public Text toComponent() {
			...
			if (hasDim) {
				if (hasCenter) {
					return new TranslatableText("wdl.messages.onMapSaved.dimAndCenterKnown", dim, xCenter,
							zCenter);
				} else {
					return new TranslatableText("wdl.messages.onMapSaved.onlyDimKnown", dim);
				}
			} else {
				if (hasCenter) {
					return new TranslatableText("wdl.messages.onMapSaved.onlyCenterKnown", xCenter,
							zCenter);
				} else {
					return new TranslatableText("wdl.messages.onMapSaved.neitherKnown");
				}
			}
		}
	}
