...
import net.world.downloader.config.EnumSetting;
import net.world.downloader.config.IntSetting;
...
public final class PlayerSettings {
	...
	public static final EnumSetting<Health> HEALTH =
			new EnumSetting<>("PlayerHealth", Health.KEEP, "wdl.gui.player.health",
					Health.values(), Health::fromString);
	public static final EnumSetting<Hunger> HUNGER =
			new EnumSetting<>("PlayerFood", Hunger.KEEP, "wdl.gui.player.hunger",
					Hunger.values(), Hunger::fromString);
	public static final EnumSetting<PlayerPos> PLAYER_POSITION =
			new EnumSetting<>("PlayerPos", PlayerPos.KEEP, "wdl.gui.player.position",
					PlayerPos.values(), PlayerPos::fromString);
	...
	public enum Health implements StringIdentifiable {
		...
		private static final Map<String, Health> FROM_STRING = SettingsUtils.makeFromString(values(), t -> t.confName);
	...
	public enum Hunger implements StringIdentifiable {
		KEEP("keep", -1, -1, -1, -1),
		FULL("20", 20, 0, 5.0f, 0.0f);

		private final String confName;
		private static final Map<String, Hunger> FROM_STRING = SettingsUtils.makeFromString(values(), t -> t.confName);
	...
	public enum PlayerPos implements StringIdentifiable {
		KEEP("keep"),
		XYZ("xyz");

		private final String confName;
		private static final Map<String, PlayerPos> FROM_STRING = SettingsUtils.makeFromString(values(), t -> t.confName);
