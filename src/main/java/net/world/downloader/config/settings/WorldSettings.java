...
import net.world.downloader.config.BooleanSetting;
...
public final class WorldSettings {
	...
	public enum GameMode implements StringIdentifiable {
		...
		private static final Map<String, GameMode> FROM_STRING = SettingsUtils.makeFromString(values(), t -> t.confName);
	...
	public enum Weather implements StringIdentifiable {
		...
		private final String confName;
		private static final Map<String, Weather> FROM_STRING = SettingsUtils.makeFromString(values(), t -> t.confName);
	...
	public enum SpawnMode implements StringIdentifiable {
		...
		PLAYER("player") {
			...
			public int getX(Entity player, IConfiguration config) {
				return MathHelper.floor(player.getX());
			}
			@Override
			public int getY(Entity player, IConfiguration config) {
				return MathHelper.floor(player.getY());
			}
			@Override
			public int getZ(Entity player, IConfiguration config) {
				return MathHelper.floor(player.getZ());
			}
		},
		XYZ("xyz") {
			@Override
		...
		private final String confName;
		private static final Map<String, SpawnMode> FROM_STRING = SettingsUtils.makeFromString(values(), t -> t.confName);
	...
	public enum Time implements StringIdentifiable {
		KEEP("keep", -1),
		SUNRISE("23000", 23000),
		MORNING("0", 0),
		NOON("6000", 6000),
		...
		private final String confName;
		private static final Map<String, Time> FROM_STRING = SettingsUtils.makeFromString(values(), t -> t.confName);
