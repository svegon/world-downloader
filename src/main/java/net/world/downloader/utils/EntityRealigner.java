...
import net.world.downloader.api.IWDLMod;
import net.world.downloader.api.IWDLModDescripted;
...
public class EntityRealigner implements IEntityEditor, IWDLModDescripted {
	...
	public String[] getAuthors() {
		return new String[] {"Pokechu22", "Svegon"};
	}
	...
	public boolean shouldEdit(Entity e) {
		...
		if (e instanceof BatEntity && ((BatEntity) e).isRoosting()) {
			return false;
		}
		...
		// invisible" issue).
		return e.getX() != 0 || e.getY() != 0 || e.getZ() != 0;
	}

	// Server pos is not a part of the entity fields in 1.16.4
	@Override
	public void editEntity(Entity e) {
		e.setPos(e.lastRenderX, e.lastRenderY, e.lastRenderZ);
	}
