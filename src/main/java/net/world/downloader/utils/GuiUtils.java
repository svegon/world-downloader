...
public final class GuiUtils {
	 ...
	 */
	public static void drawGuiInfoBox(MatrixStack matrices, String text, int guiWidth, int guiHeight,
			int bottomPadding) {
		drawGuiInfoBox(matrices, text, 300, 100, guiWidth, guiHeight, bottomPadding);
	}
	 ...
	 *            The amount of space to put below the bottom of the info box.
	 */
	public static void drawGuiInfoBox(MatrixStack matrices, String text, int infoBoxWidth,
		...
		DrawableHelper.fill(matrices, infoX, infoY, infoX + infoBoxWidth,
				infoY + infoBoxHeight, 0x7F000000);

		List<String> lines = wordWrap(text, infoBoxWidth - 10);

		for (String s : lines) {
			MC.textRenderer.draw(matrices, s, infoX + 5, y, 0xFFFFFF);
	 ...
	 */
	// I had to create it from scrap as the builtin Minecraft functioned returned a List<OrderedText> instead.
