...
public class TextEntry extends GuiListEntry<TextEntry> {
	 ...
	 */
	public TextEntry(MinecraftClient mc, String text) {
	 ...
	 */
	public TextEntry(MinecraftClient mc, String text, int color) {
		this.mc = mc;
		this.text = text;
		this.color = color;
	}
	...
	public void drawEntry(int x, int y, int width, int height, int mouseX, int mouseY) {
		...
		mc.textRenderer.drawWithShadow(this.matrixStack, text, x, y + 1, color);
	}
}