...
public class GuiWDL extends WDLScreen {
	 ...
	 */
	private Text displayedTooltip = null;

	private class GuiWDLButtonList extends GuiList<GuiWDLButtonList.ButtonEntry> {
		public GuiWDLButtonList() {
			super(GuiWDL.this.client, GuiWDL.this.width, GuiWDL.this.height, 39,
		...
		private class ButtonEntry extends GuiList.GuiListEntry<ButtonEntry> {
			...
			public ButtonEntry(String key, BiFunction<Screen, WDL, Screen> openFunc, boolean needsPerms) {
				this.button = this.addButton(new ButtonDisplayGui(0, 0, 200, 20,
						new TranslatableText("wdl.gui.wdl." + key + ".name"),
						() -> openFunc.apply(GuiWDL.this, GuiWDL.this.wdl)), -100, 0);
				if (needsPerms) {
					button.setEnabled(WDLPluginChannels.canDownloadAtAll());
				}

				this.tooltip = new TranslatableText("wdl.gui.wdl." + key + ".description");
			}
	...
	public GuiWDL(@Nullable Screen parent, WDL wdl) {
		super(new TranslatableText("wdl.gui.wdl.title", WDL.baseFolderName));
		this.parent = parent;
		this.wdl = wdl;
	...
	@Override
	public void init() {
		this.worldname = this.addTextField(new WDLTextField(this.textRenderer,
				this.width / 2 - 155, 19, 150, 18, new TranslatableText("wdl.gui.wdl.worldname")));
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		String name = I18n.translate("wdl.gui.wdl.worldname");
		this.drawString(this.textRenderer, name, this.worldname.x
				- this.textRenderer.getWidth(name + " "), 26, 0xFFFFFF);
