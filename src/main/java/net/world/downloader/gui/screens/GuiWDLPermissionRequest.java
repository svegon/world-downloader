...
public class GuiWDLPermissionRequest extends WDLScreen {
	...
	public GuiWDLPermissionRequest(@Nullable Screen parent, WDL wdl) {
		super(new LiteralText("Permission request")); // XXX Untranslated
		this.parent = parent;
		this.wdl = wdl;
	}
	...
	@Override
	public void init() {
		this.list = this.addList(new TextList(this.client, this.textRenderer, width, height, TOP_MARGIN,
				BOTTOM_MARGIN));
		...
		this.requestField = this.addTextField(new WDLTextField(this.textRenderer,
				width / 2 - 155, 18, 150, 20, new LiteralText("Request")));

		this.submitButton = this.addButton(new WDLButton(
				width / 2 + 5, 18, 150, 20,
				new LiteralText("Submit request")) {
			public @Override void performAction() {
				WDLPluginChannels.sendRequests();
				setMessage(new LiteralText("Submitted!"));
			}
		});
	...
	public void anyKeyPressed() {
		if (requestField.isFocused()) {
			String request = requestField.getText();
			requestField.setEditableColor(isValidRequest(request) ? 0x40E040 : 0xE04040);
		}
	}
