...
import net.world.downloader.WorldBackup.ICustomBackupProgressMonitor;
...
public class GuiWDLBackup extends WDLScreen {
	...
	public GuiWDLBackup(@Nullable Screen parent, WDL wdl) {
		...
		this.description = new TranslatableText("wdl.gui.backup.description1").append("\n\n")
				.append(new TranslatableText("wdl.gui.backup.description2")).append("\n\n")
				.append(new TranslatableText("wdl.gui.backup.description3"));
	}
	...
	public void init() {
		...
		customBackupCommandTemplateFld = this.addTextField(new WDLTextField(textRenderer,
				width / 2 - 100, 54, 200, 20,
				new TranslatableText("wdl.gui.backup.customCommandTemplate")));
		customBackupCommandTemplateFld.setMaxLength(255);
		customBackupCommandTemplateFld.setText(this.customBackupCommandTemplate);
		customBackupExtensionFld = this.addTextField(new WDLTextField(textRenderer,
				width / 2 + 160, 54, 40, 20,
				new TranslatableText("wdl.gui.backup.customExtension")));
	...
	private Text getBackupButtonText() {
		return new TranslatableText("wdl.gui.backup.backupMode",
	...
	public void tick() {
		...
		customBackupCommandTemplateFld.setEditableColor(color);
		customBackupExtensionFld.setEditableColor(color);
	}
		...
		class BackupTestRunnable implements ICustomBackupProgressMonitor, Runnable {
			...
			public boolean shouldCancel() {
				...
				return customSettingsChanged() ||
						...
						client.currentScreen != GuiWDLBackup.this;
			}
			...
			public void run() {
				...
				try {
					...
					File optionsTxt = new File(client.runDirectory, "options.txt"); // Should exist
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if (customBackupCommandTemplateFld.isVisible()) {
			String text = I18n.translate("wdl.gui.backup.customCommandTemplate");
			int x = customBackupCommandTemplateFld.x - 3 - textRenderer.getWidth(text);
			int y = customBackupCommandTemplateFld.y + 6;
			this.drawString(textRenderer, text, x, y, 0xFFFFFF);
		}
		if (customBackupExtensionFld.isVisible()) {
			String text = I18n.translate("wdl.gui.backup.customExtension");
			int x = customBackupExtensionFld.x - 3 - textRenderer.getWidth(text);
			...
			if (customBackupExtensionFld.getText().equalsIgnoreCase("rar")) {
				...
				this.drawString(textRenderer, "ಠ_ಠ", x, y, 0xFF0000); // See some of my experiences with dealing with rar files.  Use a non-proprietary format, please, it's for your own good!
			}
		}
		if (commandInvalidReason != null) {
			...
			for (String line : lines) {
				this.drawString(textRenderer, line, 50, y, 0xFF0000);
				y += textRenderer.fontHeight;
			}
		}

		if (customBackupCommandTemplateFld.isHovered()) {
			this.drawGuiInfoBox(new TranslatableText("wdl.gui.backup.customCommandTemplate.description"), width, height, 48);
		} else if (customBackupExtensionFld.isHovered()) {
			this.drawGuiInfoBox(new TranslatableText("wdl.gui.backup.customExtension.description"), width, height, 48);
		...
		if (this.isCommandValid) {
			...
			return VersionedFunctions.createConfirmScreen((result) -> {
				if (result) {
					...
					client.openScreen(GuiWDLBackup.this);
				}
			}, new TranslatableText("wdl.gui.backup.customCommandFailed.line1"),
					new TranslatableText("wdl.gui.backup.customCommandFailed.line2"),
					new TranslatableText("gui.yes"), new TranslatableText("gui.cancel"));
		}
	}
}
