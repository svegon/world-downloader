...
public class GuiWDLPermissions extends WDLScreen {
	...
	public GuiWDLPermissions(@Nullable Screen parent, WDL wdl) {
		super(new LiteralText("Permission info")); // XXX Untranslated
		this.parent = parent;
		this.wdl = wdl;
	}
	...
	public void init() {
		...
		this.addButton(new WDLButton((this.width / 2) + 5, 18, 150, 20,
				new LiteralText("Reload permissions")) {
			public @Override void performAction() {
				// Send the init packet.
				WDLPluginChannels.sendInitPacket("Refresh?");

				setEnabled(false);
				setMessage(new LiteralText("Refreshing..."));
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if (!WDLPluginChannels.hasPermissions()) {
			this.drawCenteredString(this.textRenderer,
					...
					- this.textRenderer.fontHeight / 2, 0xFFFFFF);
		}
	}
}
