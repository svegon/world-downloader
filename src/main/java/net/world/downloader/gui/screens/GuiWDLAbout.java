...
public class GuiWDLAbout extends WDLScreen {
	...
	public void init() {
		this.addButton(new ButtonDisplayGui((this.width / 2) - 155, 18, 150, 20,
				new TranslatableText("wdl.gui.about.extensions"), () -> new GuiWDLExtensions(this)));
		this.addButton(new WDLButton((this.width / 2) + 5, 18, 150, 20,
				new TranslatableText("wdl.gui.about.debugInfo")) {
			public @Override void performAction() {
				// Copy debug info
				VersionedFunctions.setClipboardString(wdl.getDebugInfo());
				// Change text to "copied" once clicked
				this.setMessage(new TranslatableText("wdl.gui.about.debugInfo.copied"));
			}
		});
		...
		list = this.addList(new TextList(this.client, this.textRenderer, width, height, 39, 32));
		list.addLine(I18n.translate("wdl.gui.about.blurb"));
		list.addBlankLine();
		list.addLine(I18n.translate("wdl.gui.about.version", wdlVersion,
				mcVersion));
		list.addBlankLine();

		String currentLanguage = wdl.minecraft.getLanguageManager().getLanguage().toString();
		String translatorCredit = I18n.translate("wdl.translatorCredit",
				currentLanguage);
		...
		list.addLinkLine(I18n.translate("wdl.gui.about.forumThread"), FORUM_THREAD);
		list.addBlankLine();
		list.addLinkLine(I18n.translate("wdl.gui.about.allSrc"), ALL_GITHUB);
		list.addBlankLine();
		list.addLinkLine(I18n.translate("wdl.gui.about.license"), MMPLV2);

		if (VersionConstants.shadesFastUtil()) {
			list.addBlankLine();
			list.addLine(I18n.translate("wdl.gui.about.fastutil.blurb"));
			list.addBlankLine();
			list.addLinkLine(I18n.translate("wdl.gui.about.fastutil.website"), FASTUTIL_PAGE);
			list.addLinkLine(I18n.translate("wdl.gui.about.fastutil.license"), APACHE_LICENSE_2_0);
		}
	}
	...
	protected void renderTitle(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.textRenderer, this.title.getString(), this.width / 2, 2, 0xFFFFFF);
	}
}
