...
public class GuiWDLMultiworld extends WDLScreen {
	...
	public void init() {
		String multiworldMessage = I18n
				.translate("wdl.gui.multiworld.descirption.requiredWhen")
				+ "\n\n"
				+ I18n.translate("wdl.gui.multiworld.descirption.whatIs");

		infoBoxWidth = 320;
		infoBoxLines = wordWrap(multiworldMessage, infoBoxWidth - 20);
		infoBoxHeight = (this.textRenderer.fontHeight * (infoBoxLines.size() + 1)) + 40;
		...
		this.addButton(new WDLButton(this.width / 2 - 155,
				this.height - 29, 150, 20, new TranslatableText("gui.cancel")) {
			public @Override void performAction() {
				callback.onCancel();
			}
		});
		...
		this.addButton(new WDLButton(this.width / 2 + 5,
				this.height - 29, 150, 20, new TranslatableText("gui.done")) {
			public @Override void performAction() {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		for (String s : infoBoxLines) {
			this.drawString(this.textRenderer, s, x, y, 0xFFFFFF);
	 ...
	 */
	private Text getMultiworldEnabledText() {
		return new TranslatableText("wdl.gui.multiworld." + enableMultiworld);
	}
