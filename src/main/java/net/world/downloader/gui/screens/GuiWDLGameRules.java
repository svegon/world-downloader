...
public class GuiWDLGameRules extends WDLScreen {
	 ...
	 * modified rule.
	 */
	private static final int SET_TEXT_FIELD = 0xE0E0E0;
	private static final int DEFAULT_TEXT_FIELD = 0x808080;
	...
	private class GuiGameRuleList extends GuiList<GuiWDLGameRules.GuiGameRuleList.RuleEntry> {
		...
		public GuiGameRuleList() {
			super(GuiWDLGameRules.this.client, GuiWDLGameRules.this.width,
			...
			for (String rule : vanillaGameRules.keySet()) {
				VersionedFunctions.GameRuleType type = VersionedFunctions.getRuleType(rules, rule);
		...
		private abstract class RuleEntry extends GuiList.GuiListEntry<RuleEntry> {
			...
			public RuleEntry(@Nonnull String ruleName) {
				...
				resetButton = this.addButton(new WDLButton(0, 0, 50, 20,
						new TranslatableText("wdl.gui.gamerules.resetRule")) {
			...
			public void drawEntry(int x, int y, int width, int height, int mouseX, int mouseY) {
				...
				drawString(textRenderer, this.ruleName, x, y + 6, 0xFFFFFFFF);

				if (this.isControlHovered()) {
					String key = "wdl.gui.gamerules.rules." + ruleName;
					if (I18n.hasTranslation(key)) { // may return false for mods
						hoveredToolTip = new TranslatableText(key);
					}
				}
			}
		...
		private class IntRuleEntry extends RuleEntry {
			...
			public IntRuleEntry(String ruleName) {
				...
				field = this.addTextField(new GuiNumericTextField(
						textRenderer, 0, 0, 100, 20,
						new TranslatableText("wdl.gui.gamerules.ruleValue", ruleName)),
						0, 0);
			...
			public void drawEntry(int x, int y, int width, int height, int mouseX, int mouseY) {
				...
				if (isRuleNonDefault(this.ruleName)) {
					field.setEditableColor(SET_TEXT_FIELD);
				} else {
					field.setEditableColor(DEFAULT_TEXT_FIELD);
				}
			}
		...
		private class BooleanRuleEntry extends RuleEntry {
			...
			public BooleanRuleEntry(String ruleName) {
				...
				button = this.addButton(new WDLButton(0, 0, 100, 20,
						new LiteralText(getRule(ruleName))) {
					public @Override void performAction() {
			...
			@Override
			public void drawEntry(int x, int y, int width, int height, int mouseX, int mouseY) {
				this.button.setMessage(new LiteralText(getRule(ruleName)));
				super.drawEntry(x, y, width, height, mouseX, mouseY);
			}
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.doneButton.isHovered()) {
			this.drawGuiInfoBox(new TranslatableText("wdl.gui.gamerules.doneInfo"),
					width, height, 48);
