...
public abstract class GuiTurningCameraBase extends WDLScreen {
	...
	protected GuiTurningCameraBase(WDL wdl, Text title) {
		super(title);
		this.wdl = wdl;
	}
	...
	public void init() {
		if (!initializedCamera) {
			this.cam = VersionedFunctions.makePlayer(wdl.minecraft, wdl.worldClient,
					wdl.player.networkHandler, wdl.player);
			this.cam.updatePositionAndAngles(VersionedFunctions.getEntityX(wdl.player),
					VersionedFunctions.getEntityY(wdl.player), VersionedFunctions.getEntityZ(wdl.player),
					wdl.player.yaw, 0.0F);
			this.yaw = wdl.player.yaw;
			this.oldCameraMode = VersionedFunctions.getPointOfView(wdl.minecraft.options);
			this.oldHideHud = wdl.minecraft.options.hudHidden;
			this.oldShowDebug = wdl.minecraft.options.debugEnabled;
			this.oldChatVisibility = wdl.minecraft.options.chatVisibility;
			this.oldRenderViewEntity = wdl.minecraft.cameraEntity;
	...
	@Override
	public void tick() {
		if (client.world != null && this.initializedCamera) {
			this.cam.prevPitch = this.cam.pitch = 0.0F;
			this.cam.prevYaw = this.yaw;
			this.cam.lastRenderY = this.cam.prevX = VersionedFunctions.getEntityY(this.cam);
			this.cam.lastRenderY = this.cam.prevX = VersionedFunctions.getEntityX(this.cam);
			this.cam.lastRenderY = this.cam.prevZ = VersionedFunctions.getEntityZ(this.cam);
			...
			this.cam.chunkX = MathHelper.floor(posX / 16.0D);
			this.cam.chunkY = MathHelper.floor(posY / 16.0D);
			this.cam.chunkZ = MathHelper.floor(posZ / 16.0D);
		}
	...
	private double truncateDistanceIfBlockInWay(double camX, double camZ, double currentDistance) {
		Vec3d playerPos = wdl.player.getPos().add(0, wdl.player.getEyeY(), 0);
		...
		for (int i = 0; i < 9; i++) {
			...
			BlockHitResult pos = client.world.raycast(new RaycastContext(from, to,
					RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, wdl.player));

			double distance = pos.getPos().distanceTo(playerPos);
	...
	private void activateRenderViewEntity() {
		...
		wdl.minecraft.options.hudHidden = true;
		wdl.minecraft.options.debugEnabled = false;
		wdl.minecraft.options.chatVisibility = ChatVisibility.HIDDEN;
	...
	private void deactivateRenderViewEntity() {
		...
		VersionedFunctions.restorePointOfView(wdl.minecraft.options, (Perspective) this.oldCameraMode);
		wdl.minecraft.options.hudHidden = oldHideHud;
		wdl.minecraft.options.debugEnabled = oldShowDebug;
		wdl.minecraft.options.chatVisibility = oldChatVisibility;
		wdl.minecraft.setCameraEntity(this.oldRenderViewEntity);
	}
	
	/**
	 * unavailable in the newest versions
	 * @param arg
	 */
	protected void renderDirtBackground(int arg) {
		// TODO
	}
}
