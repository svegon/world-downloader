...
public class GuiWDLMultiworldSelect extends GuiTurningCameraBase {
	private class WorldGuiButton extends WDLButton {
		...
		public void beforeDraw() {
			...
			if (info == null) {
				setMessage(new LiteralText("")); // XXX
				setEnabled(false);
			} else {
				setMessage(new LiteralText(info.displayName));
				setEnabled(true);
			}

			if (info != null && info == selectedMultiWorld) {
				fill(matrixStack, this.x - 2, this.y - 2,
	...
	private class MultiworldInfo {
		...
		public List<String> getDescription() {
			if (description == null) {
				...
				File savesFolder = new File(wdl.minecraft.runDirectory, "saves");
	...
	public void init() {
		...
		this.acceptBtn = this.addButton(new WDLButton(
				...
				new TranslatableText("wdl.gui.multiworldSelect.done")) {
			public @Override void performAction() {
		...
		this.newWorldButton = this.addButton(new WDLButton(
				...
				new TranslatableText("wdl.gui.multiworldSelect.newName")) {
			public @Override void performAction() {
				showNewWorldTextBox = true;
			}
		});

		this.newNameField = this.addTextField(new WDLTextField(this.textRenderer,
				this.width / 2 - 155, 29, 150, 20,
				new TranslatableText("wdl.gui.multiworldSelect.newName")));

		this.searchField = this.addTextField(new WDLTextField(this.textRenderer,
				this.width / 2 + 5, 29, 150, 20,
				new TranslatableText("wdl.gui.multiworldSelect.search")));
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.textRenderer,
				I18n.translate("wdl.gui.multiworldSelect.subtitle"),
		...
		if (this.searchField.getText().isEmpty() && !this.searchField.isFocused()) {
			drawString(this.textRenderer,
					I18n.translate("wdl.gui.multiworldSelect.search"),
	...
	private void drawMultiworldDescription() {
		...
		drawString(this.textRenderer, title, 5, 64, 0xFFFFFF);

		int y = 64 + this.textRenderer.fontHeight;
		for (String s : description) {
			drawString(this.textRenderer, s, 5, y, 0xFFFFFF);
