 ...
 */
package net.world.downloader.update;
...
import net.world.downloader.update.Release.HashData;
	...
	public void run() {
		try {
			if (!WDL.globalProps.getValue(MiscSettings.TUTORIAL_SHOWN)) {
				...
				TranslatableText success = new TranslatableText(
						"wdl.intro.success");
				TranslatableText mcfThread = new TranslatableText(
						"wdl.intro.forumsLink");
				mcfThread.setStyle(VersionedFunctions.createLinkFormatting(FORUMS_THREAD_USAGE_LINK));
				TranslatableText wikiLink = new TranslatableText(
						"wdl.intro.wikiLink");
				wikiLink.setStyle(VersionedFunctions.createLinkFormatting(WIKI_LINK));
				TranslatableText usage = new TranslatableText(
						"wdl.intro.usage", mcfThread, wikiLink);
				TranslatableText githubRepo = new TranslatableText(
						"wdl.intro.githubRepo");
				githubRepo.setStyle(VersionedFunctions.createLinkFormatting(GITHUB_LINK));
				TranslatableText contribute = new TranslatableText(
						"wdl.intro.contribute", githubRepo);
				TranslatableText redistributionList = new TranslatableText(
						"wdl.intro.redistributionList");
				redistributionList.setStyle(VersionedFunctions.createLinkFormatting(REDISTRIBUTION_LINK));
				TranslatableText warning = new TranslatableText(
						"wdl.intro.warning");
				warning.setStyle(warning.getStyle().withColor(Formatting.DARK_RED).withBold(true));
				TranslatableText illegally = new TranslatableText(
						"wdl.intro.illegally");
				illegally.setStyle(illegally.getStyle().withColor(Formatting.DARK_RED).withBold(true));
				TranslatableText stolen = new TranslatableText(
						"wdl.intro.stolen", warning, redistributionList, illegally);
				TranslatableText smr = new TranslatableText(
						"wdl.intro.stopModReposts");
				smr.setStyle(VersionedFunctions.createLinkFormatting(SMR_LINK));
				TranslatableText stolenBeware = new TranslatableText(
			...
			if (hasNewVersion()) {
				...
				TranslatableText updateLink = new TranslatableText(
			...
			if (VersionConstants.isUntestedVersion()) {
				TranslatableText githubIssues = new TranslatableText(
						"wdl.intro.githubRepo");
