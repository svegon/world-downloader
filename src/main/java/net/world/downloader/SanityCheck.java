...
import net.world.downloader.api.INetworkNameable;
...
enum SanityCheck {
	...
	MIXIN_GUIINGAMEMENU("wdl.sanity.mixin") {
		@Override
		public void run() throws Exception {
			if (!IBaseChangesApplied.class.isAssignableFrom(GameMenuScreen.class)) {
	...
	MIXIN_NHPC("wdl.sanity.mixin") {
		@Override
		public void run() throws Exception {
			if (!IBaseChangesApplied.class.isAssignableFrom(ClientPlayNetworkHandler.class)) {
	...
	TRANSLATION("wdl.sanity.translation") {
		@Override
		public void run() throws Exception {
			if (!I18n.hasTranslation(this.errorMessage)) {
